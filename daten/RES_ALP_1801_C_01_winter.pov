...
#declare Blechdachtex = texture{pigment{ color rgb <.65,.6,.6>*0.5} normal{bumps 0.1 scale 0.9} finish{metallic specular 0.8 roughness 1/20 reflection 0.02}}
//====OBEKTE=================================================================================
...
cylinder{<0,0,0>,<0,0,20>,0.3 translate <70,.7,0>}
 translate <10,-.7,-55>
texture{ pigment{bozo turbulence 0.3 color_map{[0.0 color rgb 0.4*0.5][0.5 color rgb .7*0.2][1 color rgb .6*0.5]} }normal { bozo 3 scale 0.05} finish { diffuse 0.9}}
 }
 ...
 object{Stall texture{Hausfarbe_l}}
		...
		box{<0,0,0>,<5,5,0.2> translate <18,5,0.1> texture{T_Wood22}}
		object{Fenstergitter rotate y*90 translate <29.4,5,13>}
        object{Fenstergitter rotate y*90 translate <29.4,5,24>}
        //
        box{<0,0,0>,<5,5,0.2> rotate y*90 translate <-.1,5,13> texture{T_Wood22}}
        box{<0,0,0>,<5,5,0.2> rotate y*90 translate <-.1,5,24> texture{T_Wood22}}
        //
 ...
 //Gehweg
 box{<0,0,0>,<6,0.05,28> translate<-8,0,-134> texture{pigment{image_map{png "dirtwall_snow.png" map_type 0 interpolate 2}rotate x*90}scale 10}}
 box{<0,0,0>,<20,0.05,7> translate<-6,0,-106> texture{pigment{image_map{png "dirtwall_snow.png" map_type 0 interpolate 2}rotate x*90}scale 10}}
...
//Bäume
 ...
 #declare RINDE = texture {
	pigment {
		...
		cubic_wave
		color_map {
			[0 color rgb <0.35, 0.35, 0.37>]
			[0.35 color rgb 0.49]
			[0.5 color rgb 0.49*2]
			[0.65 color rgb 0.21]
			[1 color rgb 0.25]
		}
...
#declare BOZO2 = pigment {
	bozo
	color_map {
		[0 color rgbf<1,0.5,0.0,0.4>*0.8]
		[1 color rgbf<1,.35,.11,0.7>*0.3]
	}
	scale 0.006666667
}
...
object { eastern_cottonwood_13_stems
         translate <-2,0,-98> texture {Rinde}}
  //Bodenplatte
 difference{
object {bodenplatte texture{pigment{image_map{jpeg "grasstex1_schnee.jpg" map_type 0 interpolate 2}rotate x*90}scale 10}}
